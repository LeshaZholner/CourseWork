...
namespace WebApp.Client.Services.SpecializationServices
{
    class SpecializationServices : ISpecializationServices
    {
        public async Task<Specialization> GetSpecializationAsync(int id)
        {
            ...
            var response = await client.GetAsync(AppSettingsManager.Settings["Url"] + $"/api/Specializations?id={id}");
            var content = await response.Content.ReadAsStringAsync();
            return JsonConvert.DeserializeObject<Specialization>(content);
        }
